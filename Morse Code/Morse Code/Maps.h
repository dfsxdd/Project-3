...
namespace KW
{
	...
	public:
		...
		Value_Type& operator [](const Key_Type& key){
			pair<iterator, bool> ret = the_set.insert(Entry_Type(key, Value_Type()));
			iterator itr = ret.first; //get a reference to the iterator
			Entry_Type& entry(*itr); //get the entry {key,value}
			return entry.second; //get a reference to the value so we can change it
